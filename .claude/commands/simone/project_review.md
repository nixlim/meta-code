...
- **Development scripts** — verify all dev scripts follow `makefile` pattern
- **Test file organization** — check tests are in the directory the code they test is in, not scattered
...
- **Development scripts** — MUST go through `makefile`, not standalone files
- **Test files** — MUST be in the directory the code they test is in with proper naming (`*_test.go`)
- **Documentation** — MUST be in `docs/` or `.simone/01_PROJECT_DOCS/`
- **Configuration** — MUST follow established patterns (`.env.example`, `go.mod`)
- **Temporary files** — SHOULD NOT exist in committed code

**Red Flags to Identify:**

- Multiple scripts doing similar things (duplicate functionality)
- Random `.sh` or `.go` files in root directory
- Test files outside the directory the code they test is in
- Development scripts bypassing `makefile`
