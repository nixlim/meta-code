...
func (j *JSONRPCAssertions) AssertValidJSONRPCResponse(data []byte, msgAndArgs ...interface{}) {
	...
	if len(msgAndArgs) > 0 {
		assert.True(j.t, hasResult != hasError, msgAndArgs...)
	} else {
		assert.True(j.t, hasResult != hasError, "Response must have either result or error, but not both")
	}
}
...
func (j *JSONRPCAssertions) AssertValidJSONRPCError(data []byte, expectedCode int, msgAndArgs ...interface{}) {
	...
	if len(msgAndArgs) > 0 {
		require.True(j.t, ok, msgAndArgs...)
	} else {
		require.True(j.t, ok, "Error field must be an object")
	}
	...
	if len(msgAndArgs) > 0 {
		...
		require.True(j.t, ok, "Error code must be a number")
	}
...
func (m *MCPAssertions) AssertValidMCPInitialize(data []byte, msgAndArgs ...interface{}) {
	...
	if len(msgAndArgs) > 0 {
		require.True(m.t, ok, msgAndArgs...)
	} else {
		require.True(m.t, ok, "Params must be an object")
	}
	...
	if len(msgAndArgs) > 0 {
		...
		require.True(m.t, ok, "ClientInfo must be an object")
	}
